...
enPage.on('console',message=>{if(message.type()==='error') errors.push(`EN: ${message.text()}`)})
...
await enPage.locator('#fullscreen-button').click()
assert.equal(await enPage.locator('.modal-settings input[type="range"]').count(),4)
const englishSettings=await enPage.locator('.modal-settings').innerText()
for (const text of ['Settings','Greenhouse sounds','Master volume','Music and ambience','Flowers and skills','Interface buttons','Preview a drop']) {
  assert.ok(englishSettings.includes(text),`English settings include: ${text}`)
}
assert.equal(/[А-Яа-яЁё]/.test(englishSettings),false,'English settings contain no untranslated Russian copy')
await enPage.waitForTimeout(350)
await enPage.screenshot({path:'artifacts/qa/english-settings.png'})
await enPage.locator('.modal-settings [data-close]').click()
for (const [key,expected] of [['nectar','Nectar rewards completed runs'],['daily','A new shared base sequence']]) {
  await enPage.locator(`[data-explain="${key}"]`).click()
  const tip=await enPage.locator('#stat-tip').innerText()
  assert.ok(tip.includes(expected),`${key} explanation is fully English`)
  assert.equal(/[А-Яа-яЁё]/.test(tip),false)
  await enPage.waitForTimeout(350)
  await enPage.screenshot({path:`artifacts/qa/english-tip-${key}.png`})
  await enPage.locator('[data-tip-close]').click()
}
assert.deepEqual(errors,[])
console.log('PASS: unsupported locale falls back to English, including four-channel settings and nectar/daily explanations; local adapter starts without SDK.')
await browser.close()
